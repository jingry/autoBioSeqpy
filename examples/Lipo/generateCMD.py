...
positiveSeqFile = './examples/Lipo/data/1-212.txt'
negativeSeqFile = './examples/Lipo/data/0-211.txt'
...
positiveFeaFiles = ['./examples/Lipo/data/po-aac_pssm.txt', './examples/Lipo/data/po-DFMCA_PSSM.txt',
                    './examples/Lipo/data/po-dpc_pssm.txt', './examples/Lipo/data/po-DP_PSSM.txt', 
                    './examples/Lipo/data/po-pse_pssm.txt', './examples/Lipo/data/po-pssm400.txt', 
                    './examples/Lipo/data/po-pssm_ac.txt', './examples/Lipo/data/po-single_Average.txt', 
                    './examples/Lipo/data/po-SVD_PSSM.txt']

negativeFeaFiles = ['./examples/Lipo/data/ne-aac_pssm.txt', './examples/Lipo/data/ne-DFMCA_PSSM.txt',
                    './examples/Lipo/data/ne-dpc_pssm.txt', './examples/Lipo/data/ne-DP_PSSM.txt', 
                    './examples/Lipo/data/ne-pse_pssm.txt', './examples/Lipo/data/ne-pssm400.txt', 
                    './examples/Lipo/data/ne-pssm_ac.txt', './examples/Lipo/data/ne-single_Average.txt', 
                    './examples/Lipo/data/ne-SVD_PSSM.txt']
...
for i in range(len(positiveFeaFiles)):
    ...
    modelName = './examples/Lipo/model/%s.py' %feaName
    # assert os.path.exists(modelName)
    feaDict[feaName] = (posFile,negFile,modelName,200)

repeatTime = 5

cmdTemp = 'python running.py --dataType protein %s --dataEncodingType dict %s --dataTrainFilePaths %s --dataTrainLabel 1 0%s --dataSplitScale 0.8 --modelLoadFile examples/Lipo/model/CNN.py %s --verbose 1 --showFig 0 --outSaveFolderPath %s --savePrediction 1 --saveFig 1 --batch_size 64 --epochs 20 --shuffleDataTrain 1 --spcLen 2000 %s --modelSaveName tmpMod.json --weightSaveName tmpWeight.bin --noGPU 0 --paraSaveName parameters.txt --optimizer optimizers.Adam(lr=0.001,amsgrad=False,decay=False) --dataTrainModelInd 0 0%s'
errCMDList = []
