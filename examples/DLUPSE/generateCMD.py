...
positiveSeqFile = './examples/exosomes/data/trpos2992.txt'
negativeSeqFile = './examples/exosomes/data/trneg2961.txt'
...
positiveFeaFiles = ['./examples/exosomes/data/po-aac_pssm.txt', './examples/exosomes/data/po-DFMCA_PSSM.txt',
                    './examples/exosomes/data/po-dpc_pssm.txt', './examples/exosomes/data/po-DP_PSSM.txt', 
                    './examples/exosomes/data/po-pse_pssm.txt', './examples/exosomes/data/po-pssm400.txt', 
                    './examples/exosomes/data/po-pssm_ac.txt', './examples/exosomes/data/po-single_Average.txt', 
                    './examples/exosomes/data/po-SVD_PSSM.txt']

negativeFeaFiles = ['./examples/exosomes/data/ne-aac_pssm.txt', './examples/exosomes/data/ne-DFMCA_PSSM.txt',
                    './examples/exosomes/data/ne-dpc_pssm.txt', './examples/exosomes/data/ne-DP_PSSM.txt', 
                    './examples/exosomes/data/ne-pse_pssm.txt', './examples/exosomes/data/ne-pssm400.txt', 
                    './examples/exosomes/data/ne-pssm_ac.txt', './examples/exosomes/data/ne-single_Average.txt', 
                    './examples/exosomes/data/ne-SVD_PSSM.txt']
...
for i in range(len(positiveFeaFiles)):
    ...
    modelName = './examples/exosomes/model/%s.py' %feaName
    # assert os.path.exists(modelName)
    feaDict[feaName] = (posFile,negFile,modelName,200)

repeatTime = 5

cmdTemp = 'python running.py --dataType protein %s --dataEncodingType dict %s --dataTrainFilePaths %s --dataTrainLabel 1 0%s --dataSplitScale 0.8 --modelLoadFile examples/exosomes/model/CNN.py %s --verbose 1 --showFig 0 --outSaveFolderPath %s --savePrediction 1 --saveFig 1 --batch_size 128 --epochs 20 --shuffleDataTrain 1 --spcLen 2000 %s --modelSaveName tmpMod.json --weightSaveName tmpWeight.bin --noGPU 0 --paraSaveName parameters.txt --optimizer optimizers.Adam(lr=0.001,amsgrad=False,decay=False) --dataTrainModelInd 0 0%s'
errCMDList = []
