...
#model1_out = tf.keras.backend.permute_dimensions(model1_out, (0,2,1))
...
lstmFwOut = tf.keras.layers.LSTM(units=15,return_sequences=True)(newOut)
lstmBwOut = tf.keras.layers.LSTM(units=15,return_sequences=True,go_backwards=True)(newOut)

newOut = tf.keras.layers.Attention(causal=True)([lstmFwOut,lstmBwOut])
...
newOut = tf.keras.layers.AveragePooling1D()(newOut)

#newOut = tf.keras.layers.Cropping1D(cropping=(1,98))(newOut)
#newOut = tf.keras.layers.Flatten()(newOut)
...
#newOut = tf.keras.layers.Dense(100)(newOut)
#newOut = tf.keras.layers.Dropout(0.5)(newOut)
#newOut = tf.keras.layers.Activation('relu')(newOut)
